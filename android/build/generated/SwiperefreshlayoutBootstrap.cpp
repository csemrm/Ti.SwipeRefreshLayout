...
#define TAG "com.rkam.swiperefreshlayout"

using namespace v8;

static Persistent<Object> bindingCache;

static void Swiperefreshlayout_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Swiperefreshlayout.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::SwiperefreshlayoutBindings::lookupGeneratedInit(
...
static void Swiperefreshlayout_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Swiperefreshlayout_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Swiperefreshlayout_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::SwiperefreshlayoutBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry SwiperefreshlayoutBinding = {
	"com.rkam.swiperefreshlayout",
	Swiperefreshlayout_init,
	Swiperefreshlayout_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_rkam_swiperefreshlayout_SwiperefreshlayoutBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.rkam.swiperefreshlayout", &SwiperefreshlayoutBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::SwiperefreshlayoutBindings::lookupGeneratedInit));
}
